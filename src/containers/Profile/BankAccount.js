...
const BankAccount = ({handleBackDrop, getAccount, accountId, accounts}) => {
  ...
  const renderAccounts = (accounts) => {
    if(accounts === null) {
      ...
      return accounts.map((account) =>  <BankCards sortCode={account.sortCode} customerAccountNumber={account.customerAccountNumber} accountNumber={account.accountNumber} bvnNumber={account.bvnNumber} primaryBank={account.primaryBank} key={account.externalAccountId} routingNumber={account.routingNumber} externalAccountSubType={account.externalAccountSubType}/>)
    }
  }
